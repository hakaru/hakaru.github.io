#!/usr/bin/env python3
"""
transform.py — Transforms support HTML files to add a 2-pane sidebar layout.

Reads sidebar-config.json, sidebar-styles.css, sidebar-script.js from
the same scripts/ directory and applies transformation to all target HTML files.
"""
...
def load_resources():
    with open(os.path.join(SCRIPT_DIR, "sidebar-config.json"), encoding="utf-8") as f:
        config = json.load(f)
    with open(os.path.join(SCRIPT_DIR, "sidebar-styles.css"), encoding="utf-8") as f:
        css = f.read()
    with open(os.path.join(SCRIPT_DIR, "sidebar-script.js"), encoding="utf-8") as f:
        js = f.read()
    return config, css, js
...
def compute_lang_switch_url(rel_path, app, current_lang):
    ...
    if app_id == "ChatArchive":
        ...
        if current_lang == "ja":
            # Switch to EN
            prefix = app["prefix"]  # /ChatArchive-support
            # e.g. /ChatArchive-support/index.html -> /ChatArchive-support/en/
            # For root index: go to /en/
            # For blog: /blog/... -> /en/blog/...
            ...
            # Switch back to JA root
            prefix = app["prefix"]
            # Strip language prefix from rel_path
            path_within = rel_path[len("ChatArchive-support"):].lstrip("/")
            # Remove lang segment (en/, th/, zh-Hant/)
        ...
        # Standard apps
        prefix = "/" + app["prefix"].lstrip("/")  # with leading slash for URL
...
def build_sidebar_html(config, current_app, current_lang, active_section, lang_switch_url):
    ...
    for app in config:
        ...
        if is_current:
            ...
            # Use current page's language if available, else default to 'en'
    ...
    # Determine available languages for the current app
    ...
    for lang in available_langs:
        ...
        if lang == current_lang:
            ...
            # Compute switch URL for each non-current lang
            # For simplicity, only en<->ja for standard apps, full for ChatArchive
...
def transform_html(html, sidebar_html, css, js):
    """Apply the sidebar transformation to an HTML string."""

    # 1. Inject sidebar CSS before </style>
    css_block = "\n/* Sidebar styles */\n" + css + "\n"
    html = html.replace("</style>", css_block + "    </style>", 1)

    # 2. Remove <nav class="support-nav">...</nav>
    html = re.sub(
        r'<nav\s+class="support-nav">.*?</nav>',
        "",
        html,
        flags=re.DOTALL,
    )

    # 3. Remove <a ... class="back-link">...</a>
    html = re.sub(
        r'<a\b[^>]*\bclass="back-link"[^>]*>.*?</a>',
        "",
        html,
        flags=re.DOTALL,
    )

    # 4. After <body>: insert sidebar HTML + open content-pane div
    ...
    # 5. Before </body>: close content-pane div + inject script
    script_block = f'\n<script>\n{js}\n</script>\n'
    html = html.replace("</body>", "</div>" + script_block + "</body>", 1)
...
def main():
    ...
    for file_path in html_files:
        ...
        # Verify this language has links defined
        ...
        new_html = transform_html(html, sidebar_html, css, js)
    ...
    print(f"\nDone: {transformed} transformed, {skipped} skipped (already done), {errors} errors")
